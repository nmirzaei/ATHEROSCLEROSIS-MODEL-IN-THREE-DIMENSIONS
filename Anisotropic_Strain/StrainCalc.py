...
mesh = Mesh('Mesh.xml')   #Cut domain
mesh1 = Mesh('Mesh1.xml') #Intact domain
######################################################################
...
while counter<=M:
 ...
 #reading anisotropic displacements from the directory they are saved in.
 if counter%M1==0:
    u = Function(V,'directory/displacement/u%d.xml' %(counter))
    u1 = Function(V1,'directory/displacement/u%d.xml' %(counter))
 #######################################################################
    #Reading growth parameters by the Reader function
    if counter!=0:
    ...
    #Caculating strain
    if MC==1 and counter%M2==0:   #color maps
...
#Saving the strain maxima on csv files
if M4==1:
    import csv
