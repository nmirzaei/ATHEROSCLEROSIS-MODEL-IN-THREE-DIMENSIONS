...
#Scratch directory in case of big outputs
...
vtkfile1 = File('Evolution/Vol_rec.pvd')
vtkfile2 = File('Evolution/bnd_rec.pvd')
vtkfile3 = File('Evolution/domain_rec.pvd')
vtkfile4 = File('Evolution/Vol_int_rec.pvd')
vtkfile5 = File('Evolution/Bnd_intima_rec.pvd')
########################################################################################################################
...
while int(epsilon*1000)<=int(epsmax*1000):
    ...
    #Reading the anisotropic growth parameters calculated before
    if counter!=0:
    ...
    #Constructing the cylindrical anisotropic growth tensor from the extracted parameters.
    ...
    #The weak form
    Pi = inner(TT_i,grad(v))*dx(7)+inner(TT_m,grad(v))*dx(8)+inner(TT_a,grad(v))*dx(9)#+(sigma)*dot(deformed_N,v)*ds(8)
    ######################################################################
    ...
    if sigma>sigmax:
         ...
         File("Evolution/displacement/u%d.xml" %(counter))<<project(u,V) #Save the displacements for later process
         ######################################################################
