...
#Cratch directory if the outputs are large
os.chdir("Scrathc directory")
########################################################################################################################

#Saving the volume and boundary
file=File('Radial/Volume.pvd')
file<<Volume
file=File('Radial/bnd.pvd')
file<<bnd_mesh
########################################################################################################################
...
vtkfile1 = File('Radial/Vol_rec.pvd')
vtkfile2 = File('Radial/bnd_rec.pvd')
vtkfile3 = File('Radial/domain_rec.pvd')
vtkfile4 = File('Radial/Vol_int_rec.pvd')
vtkfile5 = File('Radial/Bnd_intima_rec.pvd')
########################################################################################################################
...
while int(epsilon*1000)<=int(epsmax*1000):
    ...
    g_i = as_tensor([[(cost_i*cost_i)+(1+EXP)*(sint_i*sint_i),(cost_i*sint_i)-(1+EXP)*(cost_i*sint_i),0],[(cost_i*sint_i)-(1+EXP)*(cost_i*sint_i),(1+EXP)*(cost_i*cost_i)+(sint_i*sint_i),0],[0,0,1]])
    G_i = (1+EXP)
    ginv_i = as_tensor([[(cost_i*cost_i)+(1/(1+EXP))*(sint_i*sint_i),(cost_i*sint_i)-(1/(1+EXP))*(cost_i*sint_i),0],[(cost_i*sint_i)-(1/(1+EXP))*(cost_i*sint_i),(1/(1+EXP))*(cost_i*cost_i)+(sint_i*sint_i),0],[0,0,1]])
    ########################################################################################################################
    ...
    if sigma>sigmax:
         ...
         File("Radial/displacement/u%d.xml" %(counter))<<project(u,V) #Save the displacements for later process
         ######################################################################
