...
#In case you are using HPC and you have large data send them to a scratch memory
os.chdir("Scratch memory directory")


#Saving the volume and boundary
file=File('Opening_Angle/Volume.pvd')
file<<Volume
file=File('Opening_Angle/bnd.pvd')
file<<bnd_mesh
########################################################################################################################
...
vtkfile1 = File('Circumferential/Vol_rec.pvd')
vtkfile2 = File('Circumferential/bnd_rec.pvd')
vtkfile3 = File('Circumferential/domain_rec.pvd')
vtkfile4 = File('Circumferential/Vol_int_rec.pvd')
vtkfile5 = File('Circumferential/Bnd_intima_rec.pvd')
########################################################################################################################
...
while int(epsilon*1000)<=int(epsmax*1000):
    ...
    g_i = as_tensor([[(1+EXP)*(cost_i*cost_i)+(sint_i*sint_i),(1+EXP)*(cost_i*sint_i)-(cost_i*sint_i),0],[(1+EXP)*(cost_i*sint_i)-(cost_i*sint_i),(cost_i*cost_i)+(1+EXP)*(sint_i*sint_i),0],[0,0,1]])
    G_i = (1+EXP)
    ginv_i = as_tensor([[(1/(1+EXP))*(cost_i*cost_i)+(sint_i*sint_i),(1/(1+EXP))*(cost_i*sint_i)-(cost_i*sint_i),0],[(1/(1+EXP))*(cost_i*sint_i)-(cost_i*sint_i),(cost_i*cost_i)+(1/(1+EXP))*(sint_i*sint_i),0],[0,0,1]])
    ########################################################################################################################
    ...
    #The weak form
    Pi = inner(TT_i,grad(v))*dx(12)+inner(TT_m,grad(v))*dx(13)+inner(TT_a,grad(v))*dx(14)
    ########################################################################################################################
    ...
    if sigma>sigmax:
         ...
         File("Circumferential/displacement/u%d.xml" %(counter))<<project(u,V) #Save the displacements for later process
         ######################################################################
