...
#Scratch memory directory in case of a big output
...
vtkfile1 = File('Opening_Angle/Vol_rec.pvd')
vtkfile2 = File('Opening_Angle/bnd_rec.pvd')
vtkfile3 = File('Opening_Angle/domain_rec.pvd')
vtkfile4 = File('Opening_Angle/Vol_int_rec.pvd')
vtkfile5 = File('Opening_Angle/Bnd_intima_rec.pvd')
########################################################################################################################
...
while int(epsilon*1000)<=int(epsmax*1000):
    ...
    #This part extracts the anisotropic growth parameters saved in the home directory by the Growth parameter codes. The extraction is done via the function Reader()
    if counter!=0:
        alpha, beta, gamma = Reader(counter)
    else:
        alpha = Constant(0)
        beta = Constant(0)
        gamma = Constant(0)
    ...
    #Setting up the anisotropic growth tensor with the extracted parameters. A rotation transformation is applied to change the cylindrical values into Cartesian
